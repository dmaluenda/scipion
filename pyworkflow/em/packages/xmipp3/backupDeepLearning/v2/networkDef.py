...
import tflearn
'''
02030:   GLOBAL test accuracy: 92.345000  auc: 0.977412
02031:   pos_mean 0.672347 pos_sd 0.128881 neg_mean 0.287941 neg_perc90 0.269034 neg_perc95 0.316465
02032:   best thr 0.270910 --> accuracy 0.955650
...
#PICK_MEM= 2* 4 * max( FC_SIZE[0], (NUM_FILTERS[-1]*4) ) // 2**((len(MAX_POOL))*2)
PICK_MEM= 2* 4 * KERNEL_SIZE[-1]* max( FC_SIZE[0], (NUM_FILTERS[-1]*4) ) // 2**(
                                              (sum([1 for elem in MAX_POOL if elem >1])-1)*2)
...
#  prev_layer = x
  ...
  network = tflearn.conv_2d(prev_layer, 16, 3, activation='relu')
  ...
  network = tflearn.conv_2d(network, 64, 3, activation='relu')
  network = tflearn.max_pool_2d(network, 2)
  network = tflearn.conv_2d(network, 80, 3, activation='relu')
  network = tflearn.max_pool_2d(network, 2)  
  network = tflearn.fully_connected(network, 512, activation='relu')
  network = tflearn.layers.core.dropout(network, 0.5)
  logits = tflearn.fully_connected(network, 2, activation='softmax')  
  y_pred = tf.nn.softmax(logits)
  learningRate= 1e-4
  optimizer = tf.train.AdamOptimizer(learning_rate= learningRate, epsilon= 1e-8)


#  prev_layer = tflearn.conv_2d(prev_layer, NUM_FILTERS[0], KERNEL_SIZE[0],activation="relu", regularizer= None)
...
#  prev_layer_flat= tflearn.fully_connected(prev_layer_flat, FC_SIZE[0], activation='relu')
#  prev_layer_flat= tflearn.layers.core.dropout (prev_layer_flat, keep_prob=DROPOUT_KEEP_PROB)   
#  prev_layer_flat= tflearn.fully_connected(prev_layer_flat, FC_SIZE[1], activation='relu')
#  prev_layer_flat= tflearn.layers.core.dropout (prev_layer_flat, keep_prob=DROPOUT_KEEP_PROB) 
...
#  prev_layer= createOneConvLayer(2, prev_layer, NUM_FILTERS[2], kernerSize=KERNEL_SIZE[2], poolingStep=MAX_POOL[2])
##  prev_layer= createOneConvLayer(3, prev_layer, NUM_FILTERS[3], kernerSize=KERNEL_SIZE[3], poolingStep=MAX_POOL[3])
##  prev_layer= createOneConvLayer(4, prev_layer, NUM_FILTERS[4], kernerSize=KERNEL_SIZE[4], poolingStep=MAX_POOL[4])
#  prev_layer= createInceptionModule(1, prev_layer, NUM_FILTERS[3] )
...
#  prev_layer_flat= createFullyConLayer(1, prev_layer_flat, FC_SIZE[0], dropout_keep_prob= None)
#  prev_layer_flat= createFullyConLayer(2, prev_layer_flat, FC_SIZE[1], dropout_keep_prob= dropout_keep_prob)
#  logits, y_pred = createLastLayer( prev_layer_flat, num_labels)
...
#    tf.summary.image("images", x, max_outputs= 32)
