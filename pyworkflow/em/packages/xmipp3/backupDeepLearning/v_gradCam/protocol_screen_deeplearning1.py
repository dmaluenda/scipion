...
#        numberOfBatches = int(ceil(ndim*self.Nepochs.get()/batchSize))
        trainData= DataManager(self._getExtraPath("inputTrueParticles.xmd"),
                               inputConsensus,
                               self._getExtraPath("inputFalseParticles.xmd"),
                               inputNegatives)
        testData= DataManager( self._getExtraPath("testTrueParticles.xmd"),
                               testPos,
                               self._getExtraPath("testFalseParticles.xmd"),
                               testNeg)
        ...
        nnet.createNetForGradCAM(*trainData.shape)
        nnet.startSessionAndInitialize(numberOfThreads)
        nnet.computeGradCAM(testData)
        raise ValueError("Debugging mode")

        nnet.close(saveModel= True)
        
###        self.predict( testPos, testNeg, computeStats=True)
###        raise ValueError("Debugging mode")

        del nnet
        
    def predict(self, testParticlesPos, testParticlesNeg, computeStats=False):
        from pyworkflow.em.packages.xmipp3.deepLearning1 import  DeepTFSupervised, DataManager
        ...
        nnet.startSessionAndInitialize(numberOfThreads)
        y_pred , labels, metadataAndId_list= nnet.predictNet(testData, computeStats)      
